...
function handleCSV(event) {
  ...
  reader.onload = function (e) {
    ...
    const dataLines = lines
      .map(line => line.trim())
      .filter(line => line.length > 0 && !line.toLowerCase().includes("x") && line.includes(","));
...
function resetForm() {
  ...
  document.getElementById("hasilOutput").innerText = "Hasil akan ditampilkan di sini...";
...
function downloadHasil() {
  ...
  const blob = new Blob([window.latestResult], { type: "text/plain;charset=utf-8" });
